...
public class LeServiceConverter extends AbstractConverter<LeService, LeServiceVo> {
	...
	public LeService toItem(LeServiceVo vo) {
		if (vo == null) {
			...
			if (vo.getChefVo() != null && this.chef)
				item.setChef(userConverter.toItem(vo.getChefVo()));
	...
	public LeServiceVo toVo(LeService item) {
		if (item == null) {
			...
			if (item.getChef() != null && this.chef) {
				vo.setChefVo(userConverter.toVo(item.getChef()));
			}
