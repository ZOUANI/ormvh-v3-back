...
@Component 
public class EvaluationConverter extends AbstractConverter<Evaluation,EvaluationVo>{ 
	...
	@Override 
 	public Evaluation toItem(EvaluationVo vo) {
 	if (vo == null) {
   	 return null;
      } else {
      	Evaluation item = new Evaluation();
...
//			     item.setUpdatedBy(userConverter.toItem(vo.getUpdatedByVo())) ;
 	 ...
 	 @Override 
 	public EvaluationVo toVo(Evaluation item) {
 		if (item == null) {
    		return null;
      	} else {
