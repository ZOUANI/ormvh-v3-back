	...
	public Status save (Status status){
	       ...
	       if(foundedStatus!=null)
	          ...
	          if(status.getCreatedBy()!=null){
				    User createdBy = userService.findByUsername(status.getCreatedBy().getUsername());
				  if(createdBy == null)
				  status.setCreatedBy(userService.save(status.getCreatedBy()));
				  else
				  status.setCreatedBy(createdBy);
			  }
	    
	          if(status.getUpdatedBy()!=null){
				    User updatedBy = userService.findByUsername(status.getUpdatedBy().getUsername());
				  if(updatedBy == null)
				  status.setUpdatedBy(userService.save(status.getUpdatedBy()));
				  else
